		...
		User.find({"user.role": "seller"}, function(err, sellerbrands){
			...
			Product.find(productMainQuery,productFieldsRetrieve, function(err, products){
				...
				if(finalproducts.length == 0){ 	// no products found for the given search and filter results
					renderSearch(finalproducts, request, response1, sellerbrands, sortby, "#", searchtext, sellerfiltersapplied, priceFilters, '');
				}
				else{
					var allproducts = [];
							...
							if(files[0]){
						        ...
						        readStream.on("end", function () {
						            ...
						            if(counter == prodArray.length){
										...
										renderSearch(allproducts, request, response, sellerbrands, sortby, "#", searchtext, sellerfiltersapplied, priceFilters, request.flash('error while loading data'));
									}
						        });
							}
							else{
